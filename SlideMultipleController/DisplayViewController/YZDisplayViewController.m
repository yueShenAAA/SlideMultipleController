...
- (UIScrollView *)contentScrollView
{
    if (_contentScrollView == nil) {
        ...
        UICollectionView *contentScrollView = [[UICollectionView alloc] initWithFrame:CGRectMake(0, 0, YZScreenW, YZScreenH) collectionViewLayout:layout];
...
- (void)viewDidLayoutSubviews
{
    ...
    if (_isInitial == NO) {
        ...
        if (self.contentView.frame.size.height == 0) {
            self.contentView.frame = CGRectMake(0, self.topMargin, YZScreenW, YZScreenH - self.topMargin);
        }
...
- (void)setUpUnderLine:(UILabel *)label
{
    ...
    if (kScreenWidth == 320) {
        self.underLine.yz_width = 12;
    } else {
        self.underLine.yz_width = 16;
    }
    self.underLine.yz_width = titleBounds.size.width/2;
    self.underLine.yz_centerX = label.yz_centerX;
    self.underLine.yz_height = 2;
    self.underLine.yz_y = 62;
#if 0
    // 最开始不需要动画
